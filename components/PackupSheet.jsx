...
const INITIAL_DATA = {
  days: [
    { id:"mon", day:"Monday", date:"4/6", events:[] },
    { id:"tue", day:"Tuesday", date:"4/7", events:[
      { id:"e1", name:"Carmichael Lynch", time:"7:05AM – 12:30PM", staff:"Jonah + Josh", espresso:"2 Machine Gooseneck\nDecaf Beans\nPowerstrip", milkAddOns:"8 Milks\n1 Box Oat\n4 Almond\n1 Cream", ice:"20 + 20 + 7 lbs\n2 Bags", syrup:"Ripples", extras:"Menu\nSugar\nStir Sticks\nStevia\nTip Jar + Venmo", cart:"White Cart", van:"Mercedes", repack:"", notes:"" },
      { id:"e2", name:"Knock Inc. / ECMC", time:"8:15AM – 1:00PM", staff:"Sam W + Peder", espresso:"Brew\nCustom", milkAddOns:"1 Gal Dairy\n1 Matcha\n1 Cream", ice:"20 + 7 lbs\n1 Bag", syrup:"Mustard Mania", extras:"TC Sleeves\nTip Jar + Venmo", cart:"Black Cart #2", van:"Mercedes", repack:"", notes:"" }
    ]},
    { id:"wed", day:"Wednesday", date:"4/8", events:[
      { id:"e3", name:"ECMC", time:"9:10AM – 3:20PM", staff:"Giri + Sam S", espresso:"2 Machine Gooseneck", milkAddOns:"8 Milks\n1 Box Oat\n4 Almond", ice:"20 + 20 + 7 lbs\n2 Bags", syrup:"Special", extras:"Menu\nSugar\nStir Sticks", cart:"White Cart", van:"Ford", repack:"", notes:"" }
    ]},
    { id:"thu", day:"Thursday", date:"4/9", events:[
      { id:"e4", name:"Knock Inc.", time:"11:50AM – 4:00PM", staff:"Sam S", espresso:"Brew", milkAddOns:"4 Almond\n1 Cream", ice:"20 + 7 lbs\n1 Bag", syrup:"", extras:"Tip Jar + Venmo", cart:"Black Cart #2", van:"Ram", repack:"", notes:"Sam S please pack for a 300. We will leave cart onsite." },
      { id:"e5", name:"Target Center – Third Day", time:"3:00PM – 10:00PM", staff:"Ben", espresso:"2 Machine Gooseneck", milkAddOns:"8 Milks\n1 Box Oat", ice:"Ice Packs\nCups", syrup:"", extras:"Gray Bin (cups, bags, rags, full water jug etc.)", cart:"CART ON SITE – load out after service", van:"Ram", repack:"", notes:"" },
      { id:"e6", name:"GCA – Lady Gaga (Day 1 of 2) SHIFT 1", time:"5:00PM – 11:00PM", staff:"Giri + Jonah", espresso:"2 Machine Gooseneck\nCustom Blend", milkAddOns:"8 Milks\n1 Matcha", ice:"20 + 20 + 7 lbs", syrup:"Ripples\nSpecial", extras:"Things you need to bring:\nTip Jar + Venmo", cart:"White Cart", van:"Personal Car", repack:"", notes:"" },
      { id:"e7", name:"GCA – Lady Gaga (Day 1 of 2) SHIFT 2", time:"8:00PM – 2:00AM", staff:"Sam W", espresso:"", milkAddOns:"", ice:"", syrup:"", extras:"ON SITE", cart:"CART ON SITE", van:"Personal Car", repack:"", notes:"" }
    ]},
    { id:"fri", day:"Friday", date:"4/10", events:[
      { id:"e8", name:"Target Center – Florence and the Machine", time:"4:00PM – 11:00PM", staff:"Jonny", espresso:"2 Machine Gooseneck", milkAddOns:"8 Milks\n1 Box Oat\n4 Almond", ice:"20 + 20 + 7 lbs\n2 Bags", syrup:"Ripples", extras:"Gray Bin (cups, bags, rags, full water jug etc.)", cart:"CART ON SITE", van:"Ford", repack:"to Jonny", notes:"" }
    ]},
    { id:"sat", day:"Saturday", date:"4/11", events:[
      { id:"e9", name:"POH – Rochester", time:"8:30AM – 3:00PM", staff:"Sam W + Peder", espresso:"Brew\nDecaf Beans", milkAddOns:"8 Milks\n1 Box Oat\n4 Almond\n1 Cream", ice:"20 + 20 + 7 lbs\n2 Bags\nIce Packs", syrup:"Reach", extras:"Menu\nSugar\nStir Sticks\nStevia\nTip Jar + Venmo", cart:"White Cart", van:"Mercedes", repack:"", notes:"" }
    ]},
    { id:"sun", day:"Sunday", date:"4/12", events:[] },
    { id:"mon2", day:"Monday", date:"4/13", events:[
      { id:"e10", name:"Windsor @ Kraus Anderson", time:"10:00AM – 2:00PM", staff:"Jonah + Ben", espresso:"2 Machine Gooseneck", milkAddOns:"4 Almond\n1 Cream\n1 Matcha", ice:"20 + 7 lbs\n1 Bag", syrup:"", extras:"Menu\nTip Jar + Venmo", cart:"Black Cart #2", van:"Ford", repack:"", notes:"" }
    ]}
  ]
};
...
const DAY_COLORS = {
  mon:  { bg: "#FFF3EE", accent: "#F24E1E" },
  mon2: { bg: "#FFF3EE", accent: "#F24E1E" },
  tue:  { bg: "#E8FFF5", accent: "#0E9F6E" },
  wed:  { bg: "#F5EEFF", accent: "#A259FF" },
  thu:  { bg: "#F0FAFF", accent: "#1ABCFE" },
  fri:  { bg: "#EEFBF5", accent: "#0ACF83" },
  sat:  { bg: "#FFF8ED", accent: "#E8850C" },
  sun:  { bg: "#EEF0FF", accent: "#5E6AD2" }
};
...
function EventRow({ event, dayId, onChange, onDelete }) {
  const colors = DAY_COLORS[dayId] || DAY_COLORS.mon;
  return (
    <tr style={{ borderBottom: "1px solid #E0E0E0" }}>
      <td style={{ padding: "10px 12px", position: "sticky", left: 0, zIndex: 2, background: "#fff", borderRight: "2px solid #E0E0E0", minWidth: "180px", maxWidth: "240px" }}>
        ...
        <button onClick={onDelete} style={{ marginTop: "6px", background: "none", border: "none", color: "#ccc", cursor: "pointer", fontSize: "11px", padding: "2px 6px", borderRadius: "4px", transition: "all 0.15s" }}
...
export default function PackupSheet() {
  ...
  const [expandedDays, setExpandedDays] = useState(
    Object.fromEntries(INITIAL_DATA.days.map((d) => [d.id, true]))
  );
  ...
  // Sort days so today is always first, then the rest follow in order
  const sortedDays = (() => {
    const today = new Date();
    const todayDay = today.toLocaleDateString("en-US", { weekday: "long" }).toLowerCase();
    const todayDate = `${today.getMonth() + 1}/${today.getDate()}`;

    // First try to match by date string (e.g. "4/7")
    let todayIndex = data.days.findIndex((d) => d.date === todayDate);

    // If no date match, try matching by day name
    if (todayIndex === -1) {
      todayIndex = data.days.findIndex((d) => d.day.toLowerCase() === todayDay);
    }

    if (todayIndex <= 0) return data.days; // already first or not found
    return [...data.days.slice(todayIndex), ...data.days.slice(0, todayIndex)];
  })();
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', system-ui, sans-serif", background: "#F5F5F5", minHeight: "100vh", color: "#1E1E1E" }}>
      ...
      <div style={{ background: "#1E1E1E", padding: "14px 28px", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 4px 20px rgba(0,0,0,0.3)", borderBottom: "1px solid #333", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "14px", minWidth: "220px", justifyContent: "flex-end" }}>
          <div style={{ display: "flex", gap: "16px", padding: "8px 16px", background: "rgba(255,255,255,0.08)", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.12)" }}>
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "20px", fontWeight: 700, color: "#1ABCFE", fontFamily: "'Space Mono', monospace" }}>{data.days.length}</div>
      ...
      <div style={{ padding: "24px 28px 60px" }}>
        {sortedDays.map((day) => {
          const colors = DAY_COLORS[day.id] || DAY_COLORS.mon;
      ...
      <div style={{ textAlign: "center", padding: "16px", color: "#999", fontSize: "11px", fontFamily: "'Space Mono', monospace", borderTop: "1px solid #E0E0E0" }}>
        Click any cell to edit {"\u00B7"} Changes are live
      </div>
    </div>
  );
}
